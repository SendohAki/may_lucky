...
def login():
    ...
    for s in username:
        if s in " ~!@#$%^&*()_+-*/<>,.[]\/":
            return redirect_with_msg('/regloginpage/', u'用户名中不能包含空格或特殊字符', 'reglogin')
    if len(username) > 10:
        return redirect_with_msg('/regloginpage/', u'用户名长度不能大于15', 'reglogin')
    if password == '':
        return redirect_with_msg('/regloginpage/', u'密码不能为空', 'reglogin')
    for s in password:
        if s in " ~!@#$%^&*()_+-*/<>,.[]\/":
            return redirect_with_msg('/regloginpage/', u'密码中不能包含空格或特殊字符', 'reglogin')
    if len(password) > 15:
        return redirect_with_msg('/regloginpage/', u'密码长度不能大于15', 'reglogin')
...
def reg():
    ...
    '''for s in username:
        if not s.isalnum():
            return redirect_with_msg('/regloginpage/', u'用户名只能由数字或字母组成', 'reglogin')'''
    ...
    salt = '.'.join(random.sample('01234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', 10))
    m = hashlib.md5()
